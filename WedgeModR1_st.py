...
def create_well_log_plot(df, layer1_depth, layer2_depth, layer3_depth):
    fig = go.Figure()
    ...
    for depth, color in zip([layer1_depth, layer2_depth, layer3_depth], ['red', 'green', 'purple']):
        fig.add_hline(
            y=depth,
...
if uploaded_file is not None:
    try:
        ...
        if all(col in df.columns for col in ['Depth', 'Vp', 'Vs', 'Density']):
            ...
            # Display well log visualization
            st.subheader("Well Log Visualization")
            well_log_fig = create_well_log_plot(df, layer1_depth, layer2_depth, layer3_depth)
