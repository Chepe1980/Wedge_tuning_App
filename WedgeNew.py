...
# CWT parameters
st.sidebar.subheader("Time-Frequency Analysis")
cwt_scales = st.sidebar.slider("CWT Scales Range", 1, 100, (1, 50))
cwt_wavelet = st.sidebar.selectbox("CWT Wavelet", ['morl', 'cmor', 'gaus', 'mexh'], index=0)
...
def ricker(cfreq, phase, dt, wvlt_length):
    ...
    t = np.linspace(-wvlt_length/2, (wvlt_length-dt)/2, nsamp)
...
def perform_cwt_analysis(traces, t, scales, wavelet):
    """Perform Continuous Wavelet Transform analysis"""
    # Initialize results
    cwt_result = np.zeros((len(scales), traces.shape[1]))
    
    # Process each time sample
    for i in range(traces.shape[1]):
        trace = traces[:, i]
        coeffs, _ = pywt.cwt(trace, scales, wavelet)
        cwt_result[:, i] = np.sum(np.abs(coeffs), axis=0)  # Sum across scales
        
    return cwt_result

def perform_fft_analysis(traces, dt):
    """Perform FFT analysis on traces"""
    n = traces.shape[1]
    freqs = np.fft.rfftfreq(n, dt)
    fft_result = np.zeros((traces.shape[0], len(freqs)))
    
    for i in range(traces.shape[0]):
        spectrum = np.abs(np.fft.rfft(traces[i, :]))
        fft_result[i, :] = spectrum
...
with tab1:
    # Wedge model visualization
    st.header("Seismic Wedge Model")
...
with tab2:
    ...
    fig_wavelet, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))
    
    ax1.plot(wvlt_t, wvlt_amp, color='purple', linewidth=2)
    ax1.fill_between(wvlt_t, wvlt_amp, color='purple', alpha=0.3)
    ax1.set_title("Wavelet in Time Domain")
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Amplitude")
    ax1.grid(True)
    
    # Plot wavelet in frequency domain
    n = len(wvlt_amp)
    freq = np.fft.rfftfreq(n, d=dt)
    spectrum = np.abs(np.fft.rfft(wvlt_amp))
    
    ax2.plot(freq, spectrum, color='blue', linewidth=2)
    ax2.set_title("Wavelet Frequency Spectrum")
...
with tab3:
    ...
    # Select trace for analysis
    selected_trace = st.slider("Select trace for analysis", 0, nmodel-1, nmodel//2)
    
    # Perform CWT analysis
    scales = np.arange(cwt_scales[0], cwt_scales[1]+1)
    cwt_result = perform_cwt_analysis(syn_zo[selected_trace:selected_trace+1], t, scales, cwt_wavelet)
    
    # Perform FFT analysis
    freqs, fft_result = perform_fft_analysis(syn_zo[selected_trace:selected_trace+1], dt)
    
    # Create plots
    fig_tf, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))
    
    # CWT plot
    time_mask = (t >= min_plot_time) & (t <= max_plot_time)
    t_filtered = t[time_mask]
    cwt_filtered = cwt_result[:, time_mask]
    
    if cwt_filtered.size > 0:
        X, Y = np.meshgrid(np.arange(cwt_filtered.shape[1]), scales)
        im1 = ax1.pcolormesh(X, Y, cwt_filtered, shading='auto', cmap='jet')
        ax1.set_title(f"CWT Analysis (Trace {selected_trace})")
        ax1.set_xlabel("Time Sample")
        ax1.set_ylabel("Scale")
        plt.colorbar(im1, ax=ax1, label='Magnitude')
    else:
        ax1.text(0.5, 0.5, "No CWT data available", ha='center', va='center')
    
    # FFT plot
    if fft_result.size > 0:
        ax2.plot(freqs, fft_result[0], color='blue', linewidth=2)
        ax2.set_title(f"FFT Analysis (Trace {selected_trace})")
        ax2.set_xlabel("Frequency (Hz)")
        ax2.set_ylabel("Amplitude")
        ax2.grid(True)
    else:
        ax2.text(0.5, 0.5, "No FFT data available", ha='center', va='center')
    
    st.pyplot(fig_tf)

with tab4:
    # Amplitude analysis
    st.header("Amplitude Analysis")
    
    fig_amp, ax = plt.subplots(figsize=(10, 6))
    
    # Plot amplitude vs thickness for the upper interface
    ax.plot(np.arange(nmodel) * dz_step + dz_min, syn_zo[:, lyr_indx[:,0]], 'b-', label='Upper interface')
    
    # Plot amplitude vs thickness for the lower interface if available
    if lyr_indx.shape[1] > 1:
        ax.plot(np.arange(nmodel) * dz_step + dz_min, syn_zo[:, lyr_indx[:,1]], 'r-', label='Lower interface')
    
    ax.axvline(tuning_thickness, color='k', linestyle='--', label=f'Tuning thickness ({tuning_thickness:.1f} m)')
    ax.set_xlabel('Thickness (m)')
    ax.set_ylabel('Amplitude')
    ax.set_title('Amplitude vs. Thickness')
    ax.grid(True)
    ax.legend()
